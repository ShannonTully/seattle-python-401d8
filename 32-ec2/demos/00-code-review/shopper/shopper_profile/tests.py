...
class ProfileRouteTests(TestCase):
    ...
    def test_profile_route_returns_404_if_user_does_not_exist_and_authenticated(self):
        c = Client()
        c.force_login(self.gandalf)
        response = c.get('/profile/does_not_exist')
        c.logout()
        self.assertEqual(response.status_code, 404)
    ...
    def test_user_can_see_another_users_public_products(self):
        c = Client()
        c.login(username=self.gandalf, password='password')
        # c.force_login(self.gandalf)
        response = c.get('/profile/bilbo')
        c.logout()

        self.assertIn(b'You\'re viewing the profile for bilbo', response.content)
        self.assertContains(response, 'You\'re viewing the profile for bilbo')
        for product in response.context['products']:
            self.assertIn('PUBLIC', product.published)
            self.assertNotIn('PRIVATE', product.published)
            self.assertNotIn('SHARED', product.published)
